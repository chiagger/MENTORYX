...
import { activateChat, appendMessage } from '../../Presenter/EventsPresenter/chatListStudenteEventsPresenter';
...
async function openChat(chatObj, studente, ascoltatore) {
    ...
    titolo.innerHTML = "Chat con: " + ascoltatore.firstName + " " + ascoltatore.lastName;
...
function displayMessages(messagesArray, studente) {

    var whoami;
    for (let i = 0; i < messagesArray.length; i++) {
        if (messagesArray.at(i).sender.email !== studente.email) {
            whoami = "receiver";
        } else {
            whoami = "sender";
        }
...
function displayChats(chats) {
    ...
    chats.forEach(async (chat) => {
        ...
        nameDiv.innerHTML = ascoltatore.firstName + " " + ascoltatore.lastName;
