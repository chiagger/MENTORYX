...
async function sendMessage(chatObj, studente, ascoltatore) {
    ...
    if (message !== "") {
        appendMessage("professor", message);
        var chat = await getChatFromDatabase(chatObj.receiver, chatObj.sender);
        await insertMessageInChat(chat, message, ascoltatore, studente);
        messageInput.value = "";
        messageInput.focus();
        scrollToBottom();
    }
...
async function insertMessageInChat(chat, message, ascoltatore, studente) {
    try {
